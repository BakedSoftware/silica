...
program
  ...
  .option(
    "-s --styles [path]",
    "Directory of additional style imports relative to the src directory"
  )
  .option(
    "-i --ignore [pattern]",
    "RegExp pattern of files/folders to ignore. (Tests are ignored by default)"
  )
  .option(
    "-a --additional [path]",
    "Directory of additional JS imports relative to the src directory"
  )
  .option("-m --source-map [bool]", "Create a source map (default = false)")
  .option(
    "-o --optimization-level [int]",
    "Optimization level (0 = bundle, 1=simple, 2=advanced)"
  )
  .option(
    "-n --node <modules>",
    "Comma separated list of node modules package.json paths to include"
  )
  .option(
    "-c --compression-level [int]",
    "Compression level (0 = none, 1 = gzip, 2 = brotli (default))"
  )
  .parse(process.argv);
...
if (program.additional && program.additional !== "") {
  fs.copySync(
    path.join("src", program.additional),
    path.join(cachePath, "__additional_sources__")
  );
}
...
function preprocessView(readPath, writePath) {
  ...
  while ((match = includeRegExp.exec(content)) !== null) {
    ...
    var replacement =
      ...
      tagClosing +
      ">" +
      fs.readFileSync(path.join(cachePath, "src", fileName), "utf8");
...
function afterScriptCaller() {
  ...
  if (asyncLock === 2) {
    ...
    if (afterScriptResult.error) {
      console.log(
        "An error occurred running the after script, make sure it is executable"
      );
...
closureCompiler.run(function (exitCode, stdOut, stdErr) {
  ...
  if (program.sourceMap) {
    ...
    spawnSync("cp", ["-R", cachePath, "build/js/build_cache"], {
      stdio: [0, 1, 2],
      cwd: process.cwd(),
    });
  } else {
...
function stylusRender() {
  var styles = walk(path.join(cachePath, "src", "styles")).filter(function (
    name
  ) {
    return (
      name[0] !== "." &&
...
function writeStyles() {
  ...
  if (hasVar !== 0) {
    console.warn(
      "There may be a fault in the css near: ",
      totalCSS.substr(hasVar, 10)
    );
  }
  fs.writeFileSync(
    path.join("build", "css", "styles.css"),
    csso.minify(totalCSS).css
  );
  ...
  fs.copySync(
    path.join("node_modules", "silica", "dist"),
    path.join("build", "node_modules", "silica", "dist")
  );
  afterScriptCaller();
}
